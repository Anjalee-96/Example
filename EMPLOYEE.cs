...
namespace AmalkaFlora
{
    class EMPLOYEE
    {
        ...
        //Make a leave
        public bool makeLeave(int ID, DateTime LDate)
        {
            MySqlCommand command = new MySqlCommand("INSERT INTO `ex7`(`ID`, `LDate`) VALUES (@id,@ld)", db.getConnection);

            command.Parameters.Add("@id", MySqlDbType.Int32).Value = ID;
            command.Parameters.Add("@ld", MySqlDbType.Date).Value = LDate;
